...
public class ConsumosController : Controller
{
    ...
    {
        if (id == null || _context.Consumos == null) return NotFound();
    ...
    public async Task<IActionResult> Create([Bind("Id,Descricao,Data,Valor,Km,Tipo,VeiculoId")] Consumo consumo)
    {
        ...
        // Se não estiver válido, logamos cada erro do ModelState
        Console.WriteLine("ModelState inválido! Erros detalhados:");
        foreach (var key in ModelState.Keys)
        {
            var errors = ModelState[key].Errors;
            foreach (var error in errors) Console.WriteLine($"Propriedade: {key}, Erro: {error.ErrorMessage}");
        }
    ...
    public async Task<IActionResult> Edit(int id, [Bind("Id,Descricao,Data,Valor,Km,Tipo,VeiculoId")] Consumo consumo)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            {
                if (!ConsumoExists(consumo.Id)) return NotFound();
    ...
    {
        if (_context.Consumos == null) return Problem("Entity set 'AppDbContext.Consumos'  is null.");
        var consumo = await _context.Consumos.FindAsync(id);
        if (consumo != null) _context.Consumos.Remove(consumo);
